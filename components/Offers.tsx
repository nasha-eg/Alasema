...
export const Offers: React.FC<{ offers?: Offer[], settings: SiteSettings, lang: Language }> = ({ offers = initialOffers, settings, lang }) => {
  ...
  return (
    <section id="العروض" className={`py-24 bg-black relative overflow-hidden ${lang === 'ar' ? 'text-right' : 'text-left'}`}>
      ...
      <div className="max-w-7xl mx-auto px-6">
        <div className="text-center mb-16">
          ...
          <h2 className="text-4xl md:text-6xl font-black text-white tracking-tighter">
            {lang === 'ar' ? 'عروض' : ''} <span className="italic text-slate-500">{settings.brandName[lang]} {lang === 'en' ? 'Exclusive Offers' : 'الحصرية'}</span>
          </h2>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {activeOffers.map((offer) => (
            <div key={offer.id} className="group relative bg-[#0a0a0a] border border-orange-600/20 rounded-[2.5rem] p-10 overflow-hidden hover:border-orange-600 transition-all duration-500">
              ...
              <div className="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                <div className="space-y-4">
                  <div className={`flex items-center space-x-reverse space-x-3 ${lang === 'en' ? 'flex-row' : ''}`}>
                    <span className="bg-orange-600 text-black text-[10px] font-black px-3 py-1 rounded-lg uppercase">{offer.type[lang]}</span>
                    <span className="text-orange-500 text-xs font-bold">{offer.expiry[lang]}</span>
                  </div>
                  <h3 className="text-3xl font-black text-white">{offer.title[lang]}</h3>
                  <p className="text-slate-500 text-sm font-light leading-relaxed max-w-sm italic">"{offer.description[lang]}"</p>
                </div>
                
                <div className="flex flex-col items-center justify-center bg-white/5 border border-white/10 p-6 rounded-3xl min-w-[140px] group-hover:bg-orange-600 group-hover:border-orange-600 transition-all duration-500">
                  <span className="text-3xl font-black text-white group-hover:text-black transition-colors">{offer.discount[lang]}</span>
                  <span className="text-[9px] font-bold uppercase tracking-widest text-slate-500 group-hover:text-black/60 mt-1">
                    {lang === 'ar' ? 'وفر الآن' : 'Save Now'}
                  </span>
                </div>
              </div>

              <div className={`mt-8 pt-8 border-t border-white/5 flex ${lang === 'ar' ? 'justify-end' : 'justify-start'}`}>
                <a 
                  href={`https://wa.me/${settings.whatsapp}?text=Inquiry about ${offer.title[lang]}`}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="text-xs font-black text-white uppercase tracking-widest flex items-center space-x-reverse space-x-3 hover:text-orange-500 transition-colors"
                >
